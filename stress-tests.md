...
| 10 | `reviewing-the-diff` | Motion A: `fresh-eyes-review` and `ponytail-review` dispatched concurrently over the same diff; **risk surface → second independent reviewer pass over the auth files** (different subagent). Both lenses' output recorded verbatim to `.ratchet/review/<task-id>-<lens>.md` before triage; findings triaged, each one's disposition written back. Critical/Important fixed and re-checked. |
