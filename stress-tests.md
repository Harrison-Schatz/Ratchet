...
| 5 | `landing-the-change` (+ `writing-the-changelog` inline) | Convention: direct commit to default branch is legitimate for Tier 0. Tier 0 has no fan-out: `writing-the-changelog` runs inline — the entry rides the same commit, and `CHANGELOG.md` is created first if the repo has none. Commit `docs: fix typo in README ("teh" → "the"; grep verified clean)`. Step 4: retro only if a surprise occurred — none → skipped **by rule**. |

**Artifacts on disk afterward:** one commit — the fix, its changelog entry, and `CHANGELOG.md` itself if the repo had none. No `.ratchet/`, no brief, no plan, zero questions asked.
...
| 10 | `reviewing-the-diff` | Motion A: `fresh-eyes-review` and `ponytail-review` dispatched concurrently over the same diff; **risk surface → second independent reviewer pass over the auth files** (different subagent). Each lens appends its own findings to `.ratchet/review/<task-id>-<lens>.md` before returning; findings triaged with dispositions. Critical/Important fixed and re-checked. |
...
| 12 | `landing-the-change` (+ the record fan-out, `retrospecting`) | Convention detected (repo has PRs). **Record fan-out:** `syncing-the-docs` (docs vs diff: new `/login` + callback routes, OAuth env vars), `writing-the-changelog` (the entry, and the bump mapped from user-facing consequence) and `writing-the-issue` (declined findings, deferred scope) dispatched concurrently as sub-agents — write-sets disjoint — while `retrospecting` mines the record in the main session. Each agent's edits verified, then docs + entry + records + code committed together → push + PR (body lifted from brief + evidence, plus a Documentation section). Worklog `done`; STATE → idle. |
...
**Artifacts afterward:** `.ratchet/{STATE.md(idle), WORKLOG.md, LESSONS.md, briefs/…, plans/…, review/…}`, test infrastructure, characterization + feature tests, seam commit, feature commits, PR.
...
| 5 | `reviewing-the-diff` → `verifying-done` → `landing-the-change` → `retrospecting` | As in scenario 2 — including the record fan-out at landing (docs, changelog and records agents dispatched as sub-agents while the retro runs in the main session), minus the second reviewer pass (no risk surface). |
...
| 3 | `replanning-on-surprise` Step 3 | Steps 7–10 struck through in the plan (kept, marked invalidated). Salvage audit: steps 1–6 (clients, models, dry-run) survive — they're committed, verified clicks. Replacement steps written for timestamp-window sync (files + proofs, planning-the-work rules + self-check on remainder). Change log entry: old → new → why. Task state file and roster row updated (step counts, Next action). |
...
| # | Skill | What happens |
|---|---|---|
| 1 | `using-ratchet` | Step 0: `.ratchet/STATE.md` lists the task; its state file shows phase `executing`, `step 3 of 9` recorded as done, Next action = "Run step 4 (conflict resolver)". → `resuming-work` immediately; not treated as a new task. |
