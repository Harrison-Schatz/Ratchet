...
2. Remove this task's row from the roster (STATE.md) and archive/delete its `.ratchet/state/<task-id>.md`. The `done` entry from step 1 is the permanent landing record, so the roster needs no holding area for it. The task's `.ratchet/worklog/<task-id>.md` STAYS in place as the permanent record (the journal is not deleted; only the ephemeral state snapshot is). Do NOT flip a global idle while other tasks are active; only THIS task closes. (Tier 3 with milestones remaining: keep the row and the state file; point the state file's "Next action" at the next milestone.)
