...
| Schema or data-invariant change on minimal or empty fixtures | **Declared populated proof**: empty tables pass every migration, so run the change against a realistic snapshot (a restored backup, a seeded copy) and record what it could have FAILED on. |
