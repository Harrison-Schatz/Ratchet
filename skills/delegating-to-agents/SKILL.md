...
3. **Constraints** — files it may touch; files it must NOT touch; discipline expected (`test-first per RED-GREEN`); "do NOT fix unrelated things you find — report them." Then the clauses that only matter when delegates coexist, written INTO the brief rather than hoped for: "write nothing into the working tree — scratch in your own temp dir"; when more than one checkout exists, WHICH one to read and verify against; and when delegates share one test environment (a test database, a dev stack, a port), exactly ONE of them runs the suite — every other delegate gets read-only analysis or targeted test ids, plus "never report a red you did not reproduce alone." Concurrent runs corrupt each other and surface as failures in files nobody touched.
...
1. Look at the actual diff (`git diff` / read the files). Changes exist? Within the allowed file set? Stray files (`git status --short`) are the delegate's too — a probe script left in the tree ships with the next `add -A`; sweep before the checkpoint.
